...
export class AcpHarnessDriver {
  ...
  constructor(acp, { versionProcess, snapshotDirectory, historyLoader } = {}) {
    this.#acp = acp
  ...
  async health() {
    await this.start()
    if (!this.#version && this.#versionProcess) {
      this.#version = execFile(this.#versionProcess.command, this.#versionProcess.args, { timeout: 10_000 })
        .then(({ stdout }) => stdout.trim().replace(/^v/, "") || this.agentInfo?.version || "unknown")
        .catch(() => this.agentInfo?.version || "unknown")
    }
    return { version: await (this.#version ?? this.agentInfo?.version ?? "unknown") }
  }
