...
assert.ok(create.includes('const supportedTransport'), 'OpenCode native create must remain on the shared native transport gate')
...
assert.ok(adapter.includes('reconcileNativeTranscriptStatus(entry, page, before)'), 'OpenCode and OMP completion must reconcile from the native transcript already consumed by v3')
assert.ok(adapter.includes('entry.target.backend !== "opencode" && entry.target.backend !== "omp"'), 'transcript-driven completion must stay scoped to OpenCode and durable OMP journals')
assert.ok(adapter.includes('message.info.time?.completed'), 'native transcript completion must require terminal metadata, not assistant-text heuristics')
