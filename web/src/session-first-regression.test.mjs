...
const reconciliation = read('./native-session-reconciliation.ts')
...
assert.ok(adapter.includes('openCodeTranscriptCompletion'), 'OpenCode turn matching must stay isolated from generic projection state')
assert.ok(reconciliation.includes('visibleNativePrompt'), 'native handoff prompt normalization must stay in the reconciliation domain')
assert.ok(reconciliation.includes('message.info.time?.completed'), 'OpenCode transcript completion must require native terminal metadata, not assistant-text heuristics')
