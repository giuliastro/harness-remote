...
const homeBase = read('./components/native-session-home-base.tsx')
const attentionInbox = read('./components/native-session-home-attention.tsx')
...
assert.match(home, /native-session-home-attention/, 'the public Session rail entrypoint must compose the global Attention Inbox')
assert.match(homeBase, /hr-native-machine-group/, 'Session navigation must preserve machine grouping')
assert.match(homeBase, /hr-native-project-group/, 'Session navigation must preserve Project grouping')
...
assert.match(attentionInbox, /baseAttentionCount \+ inbox\.length/, 'mobile navigation must carry explicit global attention as well as legacy rail attention')
