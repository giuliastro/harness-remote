...
const reconciliation = readFileSync(new URL('./native-session-reconciliation.ts', import.meta.url), 'utf8')
...
assert.ok(adapter.includes('stabilizePiTailMessageIDs'), 'PI tail reads must delegate identity stabilization to the reconciliation domain')
assert.ok(reconciliation.includes('candidates?.length !== 1'), 'PI identity stabilization must refuse ambiguous repeated-answer matches')
assert.ok(reconciliation.includes('nextKeyCounts.get(key) !== 1'), 'PI identity stabilization must preserve legitimate repeated identical journal answers')
...
assert.ok(reconciliation.includes('message.info.error'), 'PI identity stabilization must keep interrupted/error turns outside text-only aliasing')
...
assert.ok(reconciliation.includes('lastNativeMessageModel(page.messages)'), 'OpenCode tail reconciliation must recover the newest native turn model')
assert.ok(adapter.includes('nativeSessionTranscriptModel(entry.target.backend, page, before)'), 'adapter must delegate transcript model authority to the reconciliation domain')
assert.ok(adapter.includes('openCodeTranscriptCompletion'), 'adapter must delegate OpenCode terminal-turn matching to the reconciliation domain')
